...
TEST_SUITE("SPSCClient")
{
    TEST_CASE("Correctly sends gauge metrics")
    {
        auto received = false;
        {
            ...
            client.send(std::move(Gauge("gauge.name", 43.0).with(Tags::from_tags({"tag1:hello", "tag2:world"}))));
    ...
    TEST_CASE("Can correctly send gauge metrics after being moved")
    {
        auto received = false;
        {
            ...
            moved.send(std::move(Gauge("gauge.name", 43.0).with(Tags::from_tags({"tag1:hello", "tag2:world"}))));
            barrier.arrive_and_wait();
        }
        CHECK(received);
    }
